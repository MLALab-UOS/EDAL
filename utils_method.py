...
def remove_pooled_points(pool_data, pool_target, pool_subset_dropout, pool_data_dropout, pool_target_dropout, pool_index, args):
    ...
    tmp_isCifar10 = args.isCifar10_pool.copy()

    np_data = np.delete(np_data, pool_subset_idx, axis=0)
    np_target = np.delete(np_target, pool_subset_idx, axis=0)
    args.isCifar10_pool = np.delete(args.isCifar10_pool, pool_subset_idx, axis= 0)
    ...
    pool_subset_idx = np.delete(pool_subset_idx, np_index, axis= 0)
    ...
    args.isCifar10_pool = np.concatenate((args.isCifar10_pool, tmp_isCifar10[pool_subset_idx]), axis= 0)
